...
SNS_TOPIC_ARN = 'arn:aws:sns:ap-south-1:353250843450:TravelGoBookingTopic'  # Replace with your actual SNS topic ARN
...
def confirm_train_details(): 
    ...
    if len(available_seats) < booking_details['num_persons']: 
        flash("Not enough seats available. Please try fewer persons or a different train/date.", 
"error") 
...
def final_confirm_train_booking():
    ...
    # Re-checking seat availability with a scan for consistency
    response = bookings_table.scan(
    ...
    # Re-allocate seats based on current availability
    ...
    send_sns_notification(
        ...
        message=(
            f"Dear {booking_data['user_email']},\n"
            f"Your train booking for {booking_data['name']} (Train No: {booking_data['train_number']}) "
            f"from {booking_data['source']} to {booking_data['destination']} on {booking_data['travel_date']} is confirmed.\n"
            f"Your allocated seats are: {booking_data['seats_display']}\n"
...
def final_confirm_bus_booking(): 
    ...
    send_sns_notification(
        ...
        message=(
            ...
            f"Your bus from {booking['source']} to {booking['destination']} on {booking['travel_date']} at {booking['time']} ({booking['type']}) is confirmed.\n"
...
def confirm_flight_booking(): 
    ...
    send_sns_notification(
        ...
        message=(
            f"Dear {booking['user_email']},\n"
            f"Your flight booking on {booking['travel_date']} from {booking['source']} to {booking['destination']} with {booking['airline']} (Flight No: {booking['flight_number']}) is confirmed.\n"
            f"Total Price: ₹{booking['total_price']}"
        )
    ) 
...
def confirm_hotel_booking(): 
    ...
    send_sns_notification(
        ...
        message=(
            f"Dear {booking['user_email']},\n"
            f"Hotel booking at {booking['name']} in {booking['location']} from {booking['checkin_date']} to {booking['checkout_date']} for {booking['num_rooms']} rooms and {booking['num_guests']} guests is confirmed.\n"
            f"Total Price: ₹{booking['total_price']}"
        )
    ) 
...
def cancel_booking(): 
    ...
    try: 
        ...
        flash(f"Booking {booking_id} cancelled successfully!", category='success')
    except Exception as e:
        flash(f"Failed to cancel booking {booking_id}: {str(e)}", category='error') 
 
    return redirect(url_for('dashboard')) 
 
 
if __name__ == '__main__': 
    # IMPORTANT: In a production environment, disable debug mode and specify a host.
